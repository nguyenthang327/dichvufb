				...
				<div class="box">
					...
					<div class="box-body">
						<div class="row">
							<div class="col-lg-3 col-sm-6 col-12">
								<div class="form-group">
									...
									<select class="form-select" name="NetworkCode" id="NetworkCode">
										<option value="VIETTEL">VIETTEL (Chiết khấu <?= $chietkhau ?>%) </option>
										<option value="MOBIFONE">MOBIFONE (Chiết khấu <?= $chietkhau ?>%) </option>
										<option value="VINAPHONE">VINAPHONE (Chiết khấu <?= $chietkhau ?>%) </option>
										<option value="VIETNAMOBILE">VIETNAMOBILE (Chiết khấu <?= $chietkhau ?>%)</option>
									</select>
								</div>
							</div>
							<div class="col-lg-3 col-sm-6 col-12">
							...
							<div class="col-lg-3 col-sm-6 col-12">
								<div class="form-group">
									<button onclick="submit();" type="submit" name="submit" id="submit" class="btn btn-primary btn-block">Nạp ngay</button>
								</div>
							</div>
								...
								<div class="table-responsive ">
									<table class="table">
										...
										<tbody>
											...
											$total_records = mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `historynapcard` WHERE `webdinhdanh` = '$domain' AND `username` ='$username' $a $b"));
											$total_pages = ceil($total_records / $num_rec_per_page);
											$respawn = mysqli_query($ketnoi, "SELECT * FROM `historynapcard` WHERE `webdinhdanh` = '$domain' AND `username` ='$username' $a $b order by id desc LIMIT $start_from, $num_rec_per_page ");
											...
											?>
												...
												<?php else : while ($rowhis = mysqli_fetch_array($respawn, MYSQLI_ASSOC)) : ?>
													<tr>
														...
														<td><?= $rowhis['seri'] ?></td>
														<td><?= $rowhis['mathe'] ?></td>
														<td><?= number_format($rowhis['menhgia']) ?> VNĐ</td>
														<td><?= number_format($rowhis['thucnhan']) ?> VNĐ</td>
														<td><?= $rowhis['date'] ?></td>
														<td><?php if ($rowhis['status'] == 'pending') { ?><span class="badge badge-info"><?= $rowhis['status'] ?></span> <?php } ?>
															<?php if ($rowhis['status'] == 'success') { ?><span class="badge badge-success"><?= $rowhis['status'] ?></span> <?php } ?>
															<?php if ($rowhis['status'] == 'error') { ?><span class="badge badge-danger"><?= $rowhis['status'] ?></span> <?php } ?>
														</td>
														<td>
														</td>
													</tr>
