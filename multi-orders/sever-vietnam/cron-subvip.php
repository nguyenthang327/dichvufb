...
$kiemtratinhnang = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `danhmuccon` WHERE `webnguon` ='subgiare' AND `webdinhdanh` ='$domain' AND `id` = '33'"));
$action = $kiemtratinhnang['action'];
if($action =='on'){
$dem =  mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='33' AND `status` = 'pending'"));   
if($dem > 0){
// chống đúp đơn
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'off' WHERE `webnguon` ='subgiare' AND `webdinhdanh` ='$domain' AND `id` = '33'");
// lấy danh sách đơn hàng - giới hạn tối đa 20 đơn / phút
$datadonhang = mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='33' AND `status` = 'pending' LIMIT 20");
...
$dayvip = $row['dayvip']; 
$id = $row['id']; 
$checkgiagoc = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `chietkhau` WHERE `madichvu` ='33' AND `server` = '$server' AND `webdinhdanh` ='$domain'"));
...
$res = curl_send1("/api/service/facebook/service/facebook/sub-vip/order","https://$g/api/service/facebook/sub-vip/order",$data,$token1);
$obj = json_decode($res);
$status = $obj-> status;
$tinnhan = $obj-> message;
if($status == true){
...
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'on' WHERE `webnguon` ='subgiare' AND `webdinhdanh` ='$domain' AND `id` = '33'");
echo "Đã xử lý sub vip xong <br>";
}else{
echo 'Không có đơn sub vip nào đang chờ xử lý !';        
}
}else{
