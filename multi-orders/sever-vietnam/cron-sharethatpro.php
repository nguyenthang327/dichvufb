...
$kiemtratinhnang = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `danhmuccon` WHERE `webnguon` ='traodoisub' AND `webdinhdanh` ='$domain' AND `id` = '46'"));
$action = $kiemtratinhnang['action'];
if($action =='on'){
$dem =  mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='46' AND `status` = 'pending'"));   
if($dem > 0){
// chống đúp đơn
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'off' WHERE `webnguon` ='traodoisub' AND `webdinhdanh` ='$domain' AND `id` = '46'");
// lấy danh sách đơn hàng - giới hạn tối đa 20 đơn / phút
$datadonhang = mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='46' AND `status` = 'pending' LIMIT 20");
...
$dayvip = $row['dayvip']; 
$id = $row['id']; 
$checkgiagoc = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `chietkhau` WHERE `madichvu` ='46' AND `server` = '$server' AND `webdinhdanh` ='$domain'"));
...
$res = curl_sendtds($cookitdsv3,"https://traodoisub.com/mua/share/themid.php",$data,$refff);
...
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'on' WHERE `webnguon` ='traodoisub' AND `webdinhdanh` ='$domain' AND `id` = '46'");
echo "Đã xử lý share pro traodoisub xong <br>$res";
}else{
echo 'Không có đơn share pro traodoisub nào đang chờ xử lý !';        
}
}else{
