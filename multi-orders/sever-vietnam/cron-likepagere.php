...
$kiemtratinhnang = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `danhmuccon` WHERE `webnguon` ='subgiare' AND `webdinhdanh` ='$domain' AND `id` = '9'"));
$action = $kiemtratinhnang['action'];
if($action =='on'){
$dem =  mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='9' AND `status` = 'pending'"));   
if($dem > 0){
// chống đúp đơn
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'off' WHERE `webnguon` ='subgiare' AND `webdinhdanh` ='$domain' AND `id` = '9'");
// lấy danh sách đơn hàng - giới hạn tối đa 20 đơn / phút
$datadonhang = mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='9' AND `status` = 'pending' LIMIT 20");
...
$dayvip = $row['dayvip']; 
$id = $row['id']; 
$checkgiagoc = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `chietkhau` WHERE `madichvu` ='9' AND `server` = '$server' AND `webdinhdanh` ='$domain'"));
...
$res = curl_send1("/api/service/facebook/like-page-sale/order","https://$g/api/service/facebook/like-page-sale/order",$data,$token1);
$obj = json_decode($res);
$status = $obj-> status;
...
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'on' WHERE `webnguon` ='subgiare' AND `webdinhdanh` ='$domain' AND `id` = '9'");
echo "Đã xử lý like page rẻ xong <br>";
}else{
echo 'Không có đơn like page rẻ nào đang chờ xử lý !';        
}
}else{
