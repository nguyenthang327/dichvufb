...
$kiemtratinhnang = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `danhmuccon` WHERE `webnguon` ='autofb' AND `webdinhdanh` ='$domain' AND `id` = '27'"));
$action = $kiemtratinhnang['action'];
if($action =='on'){
$dem =  mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='27' AND `status` = 'pending'"));   
if($dem > 0){
// chống đúp đơn
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'off' WHERE `webnguon` ='autofb' AND `webdinhdanh` ='$domain' AND `id` = '27'");
// lấy danh sách đơn hàng - giới hạn tối đa 20 đơn / phút
$datadonhang = mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='27' AND `status` = 'pending' LIMIT 20");
...
$dayvip = $row['dayvip']; 
$id = $row['id']; 
$checkgiagoc = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `chietkhau` WHERE `madichvu` ='27' AND `server` = '$server' AND `webdinhdanh` ='$domain'"));
...
$data = '{"dataform":{"link":"'.$row['link'].'","profile_user":"'.$link.'","loaiseeding":"'.$server.'","startDatebh":"'.$date1.'T03:06:12.340Z","EndDatebh":"'.$date1.'T03:06:12.340Z","baohanh":0,"sltang":'.$soluong.',"giatien":'.$giagoc.',"ghichu":"","list_messages":'.$comment.',"infoTiktok":{"userid_tiktok":"'.$userid_tiktok.'","uniqueId_tiktok":"'.$uniqueId_tiktok.'","avatarThumb_tiktok":"'.$avatarThumb_tiktok.'","avatarMedium_tiktok":"'.$avatarMedium_tiktok.'","verified_tiktok":"'.$verified_tiktok.'","secUid_tiktok":"'.$secUid_tiktok.'","relation_tiktok":"'.$relation_tiktok.'","like_tiktok":'.$like_tiktok.',"playCount":'.$playCount.',"cmt_count":'.$cmt_count.',"fans_tiktok":'.$fans_tiktok.',"shareCount":'.$shareCount.',"id_video":"'.$id_video.'"}}}';
...
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'on' WHERE `webnguon` ='autofb' AND `webdinhdanh` ='$domain' AND `id` = '27'");
echo "Đã xử lý Comment tiktok xong <br>";

}else{
echo 'Không có đơn Comment tiktok nào đang chờ xử lý !';        
}
}else{
