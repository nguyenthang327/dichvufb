...
$kiemtratinhnang = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `danhmuccon` WHERE `webnguon` ='traodoisub' AND `webdinhdanh` ='$domain' AND `id` = '48'"));
$action = $kiemtratinhnang['action'];
if($action =='on'){
$dem =  mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='48' AND `status` = 'pending'"));   
if($dem > 0){
// chống đúp đơn
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'off' WHERE `webnguon` ='traodoisub' AND `webdinhdanh` ='$domain' AND `id` = '48'");
// lấy danh sách đơn hàng - giới hạn tối đa 20 đơn / phút
$datadonhang = mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='48' AND `status` = 'pending' LIMIT 20");
...
$server = $row['server'];    
$comment = $row['comment'];  
$reaction = strtoupper($row['reaction']);    
$dayvip = $row['dayvip']; 
$id = $row['id']; 
$checkgiagoc = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `chietkhau` WHERE `madichvu` ='48' AND `server` = '$server' AND `webdinhdanh` ='$domain'"));
...
$data = 'maghinho='.$code_oder.'&id='.$link.'&loaicx='.$reaction.'&speed=1&sl='.$soluong.'&dateTime='.$encode.'';
$refff = 'https://traodoisub.com/mua/reaction/';
$res = curl_sendtds($cookitdsv3,"https://traodoisub.com/mua/reaction/themid.php",$data,$refff);
...
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'on' WHERE `webnguon` ='traodoisub' AND `webdinhdanh` ='$domain' AND `id` = '48'");
echo "Đã xử lý reaction pro traodoisub xong <br>$res";
}else{
echo 'Không có đơn reaction pro traodoisub nào đang chờ xử lý !';        
}
}else{
