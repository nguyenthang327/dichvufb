...
$kiemtratinhnang = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `danhmuccon` WHERE `webnguon` ='autofb' AND `webdinhdanh` ='$domain' AND `id` = '25'"));
$action = $kiemtratinhnang['action'];
if($action =='on'){
$dem =  mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='25' AND `status` = 'pending'"));   
if($dem > 0){
// chống đúp đơn
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'off' WHERE `webnguon` ='autofb' AND `webdinhdanh` ='$domain' AND `id` = '25'");
// lấy danh sách đơn hàng - giới hạn tối đa 20 đơn / phút
$datadonhang = mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `webdinhdanh` = '$domain' AND `tinhnang`='25' AND `status` = 'pending' LIMIT 20");
...
$dayvip = $row['dayvip']; 
$id = $row['id']; 
$checkgiagoc = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `chietkhau` WHERE `madichvu` ='25' AND `server` = '$server' AND `webdinhdanh` ='$domain'"));
$giagoc = $checkgiagoc['giagoc'];
$pos = strpos($link, 'tiktok.com');
if ($pos !== false) {
$link = uid('uidtiktok',$link,$token);
}
$date1 = date("Y-m-d");    
$data = array(
"data" => "$link",
...
$referer = 'https://autofb.pro/tool/tiktokbuff';
$data2 = array(
    "dataform" => array (
        ...
        "following_tiktok" => "$following_tiktok",        
        "heart_tiktok" => $heart_tiktok,
        "openFavorite_tiktok" => $openFavorite_tiktok,
        "relation_tiktok" => $relation_tiktok,
        "secUid_tiktok" => "$secUid_tiktok",
        "secret_tiktok" => $secret_tiktok, 
...
mysqli_query($ketnoi,"UPDATE `danhmuccon` SET `action` = 'on' WHERE `webnguon` ='autofb' AND `webdinhdanh` ='$domain' AND `id` = '25'");
echo "Đã xử lý sub tiktok xong <br>";
}else{
echo 'Không có đơn sub tiktok nào đang chờ xử lý !';        
}
}else{
