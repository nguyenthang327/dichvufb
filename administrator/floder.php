...
if($action == 'del'){
mysqli_query($ketnoi,"DELETE FROM `danhmuc` WHERE `id`='$target' AND `webdinhdanh` = '$domain'");
mysqli_query($ketnoi,"DELETE FROM `danhmuctinhnang` WHERE `danhmuc`='$target' AND `webdinhdanh` = '$domain'");
	die('<script type="text/javascript">swal("Thông báo","Xóa thành công !","success"); setTimeout(function(){ location.href = "/administrator/floder.html" },2000);</script>');
}elseif($action == 'edit'){
$databak = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `danhmuc` WHERE `id` ='$target' AND `webdinhdanh` = '$domain'"));   
...
<div class="form-group">
 <input type="hidden" id="idbank" name="idbank" value="<?=$idbank?>">    
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input  type="text" class="form-control" id="icon" name="icon" placeholder='<i class="fab fa-facebook-square"></i> hoặc <img src="link ảnh" width="Kích cỡ px">' value='<?=$icon?>'>
                                    </div>
...
<button onclick="location.href='/administrator/floder.html'" type="submit" id="submit" id="submit" class="btn btn-danger btn-lg"> Reload</button>
</center>
</div>
</div>
</div> 
...
$total_records = mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `danhmuc` WHERE `webdinhdanh` = '$domain'"));
$total_pages = ceil($total_records / $num_rec_per_page);                                            
$respawn = mysqli_query($ketnoi,"SELECT * FROM `danhmuc` WHERE `webdinhdanh` = '$domain' LIMIT $start_from, $num_rec_per_page ");
...
<?php else: while ($row = mysqli_fetch_array($respawn, MYSQLI_ASSOC)):?>    
                                                <tr>
                                                    ...
                                                    <td><a class="btn btn-success" href="/administrator/floder.html?action=edit&target=<?php echo $row['id'] ?>">Edit</a> 
                                                    
                                                    <a onclick="return confirm('Hành động nguy hiểm - Hành động này có thể gây nguy hại đến tính năng trên website , xác nhận xóa nhấn OK, chưa chắc nhấn Hủy')" href="/administrator/floder.html?action=del&target=<?php echo $row['id'] ?>" class="btn btn-danger">Xóa</a>
                                                    </td>
                                                </tr>
...
else {if(!$('#sapxep')['val']()){swal('ERROR','Bạn muốn sắp xếp ra sao ?','error')}
