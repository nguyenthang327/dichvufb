...
mysqli_query($ketnoi,"UPDATE `function` SET `dachay`='$soluong',`status` ='success' WHERE `id` = '$id'");
	die('<script type="text/javascript">swal("Thông báo","Đã set trạng thái đơn hoàn thành thành công !","success"); setTimeout(function(){ location.href = "/administrator/dontay.html" },2000);</script>');
}elseif($type =='refund'){
$datadon = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `function` WHERE `id` ='".mysqli_real_escape_string($ketnoi,$id)."'  AND `webdinhdanh` = '$domain'")); 
...
`note` = 'Hoàn Tiền đơn $codeodergoc'");
	die('<script type="text/javascript">swal("Thông báo","Đã hoàn Full đơn hàng !","success"); setTimeout(function(){ location.href = "/administrator/dontay.html" },2000);</script>');
}else{
	die('<script type="text/javascript">swal("Thông báo","Tài khoản buff đơn hàng này đang thao tác trên tính năng, vui lòng thử lại sau vài giây hoặc đơn đã hoàn tiền không thể hoàn thêm !","error"); setTimeout(function(){ location.href = "/administrator/dontay.html" },2000);</script>');    
}
}elseif($type =='run'){
mysqli_query($ketnoi,"UPDATE `function` SET `status` ='pending' WHERE `id` = '$id'");
	die('<script type="text/javascript">swal("Thông báo","Đã set trạng thái đơn chờ xử lý thành công !","success"); setTimeout(function(){ location.href = "/administrator/dontay.html" },2000);</script>');
}
}
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"><i class="fas fa-shopping-bag"></i> Quản lý đơn tay</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
...
<form action="" method="POST">
                                    <div class="form-group">
                                        <label for="disabledInput">Tìm Kiếm  đơn tay</label>
...
<?php endwhile; endif; ?> 
                                    ...
                                    <div class="form-group">
                                      ...
                                      <a class="btn btn-danger" href="/administrator/dontay.html">Reset</a>
                                       <div class="float-right">
                                      <a class="btn btn-success" href="/administrator/dontay.html?status=success" role="button">Success (<?=$a1?>)</a>
                                      <a class="btn btn-danger" href="/administrator/dontay.html?status=error" role="button">Error (<?=$b1?>)</a>
                                      <a class="btn btn-info" href="/administrator/dontay.html?status=pending" role="button">Pending (<?=$c1?>)</a>
                                      <a class="btn btn-secondary" href="/administrator/dontay.html?status=inprogess" role="button">In Progress (<?=$d1?>)</a>
                                      <a class="btn btn-primary" href="/administrator/dontay.html?status=refund" role="button">Refund (<?=$e1?>)</a>
                                      </div>
                                    </div>
                                    </form>
 </div> 
...
<?php else: while ($row = mysqli_fetch_array($respawn, MYSQLI_ASSOC)):?>    
                                                <tr>
                                                   ...
                                                   $fp = fopen($filename, "r");//mở file ở chế độ đọc
                                                    ...
                                                    <td><?php if($row['status'] == 'pending'){?><span class="badge bg-info">Pending</span> <?php }elseif($row['status'] == 'success'){?><span class="badge bg-success">Success</span> <?php }elseif($row['status'] == 'refund'){?><span class="badge bg-primary">Refund</span> <?php }elseif($row['status'] == 'error'){?><span class="badge bg-danger">Error</span> <?php }elseif($row['status'] == 'pause'){?><span class="badge bg-info">Pause</span> <?php }elseif($row['status'] == 'inprogess'){?><span class="badge bg-secondary">In Progress</span> <?php } ?></td>
                                                    <td><a class="badge bg-info" href="/administrator/dontay.html?type=finish&id=<?php echo $row['id'] ?>">Hoàn thành</a> 
                                                    ...
                                                    <a onclick="return confirm('Hành động nguy hiểm - Hành động này sẽ hoàn toàn bộ tiền của đơn này cho người buff , xác nhận nhấn OK, chưa chắc nhấn Hủy')" href="/administrator/dontay.html?type=refund&id=<?php echo $row['id'] ?>" class="badge bg-danger">Hoàn full</a>
                                                    <?php }
                                                    ?>
                                                    <a  onclick="return confirm('Hành động nguy hiểm - Hành động này sẽ tiến hành gửi lại đơn trên hệ thống , xác nhận nhấn OK, chưa chắc nhấn Hủy')" href="/administrator/dontay.html?type=run&id=<?php echo $row['id'] ?>" class="badge bg-primary">Chạy lại</a>
