...
$respawnbank = mysqli_fetch_array(mysqli_query($ketnoi, "SELECT * FROM `banker` WHERE `webdinhdanh` = '$domain' AND `mabank` = 'acb'"));
...
$result = json_decode(curl_get("https://api.web2m.com/historyapiacb/$password/$sotaikhoan/$token"), true);
...
if ($soluong > 0) {
  for ($j = 0; $j < $soluong; $j++) {
    $magd =  $data[$j]['transactionNumber'];
    $amount =  $data[$j]['amount'] * $tygia / 100;
    ...
    if ($amount > $toithieu) {
      ...
      if ($dem1 == 1 && $loaihinh == 'IN') {
        ...
        $dem = mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `historynaptien` WHERE `username` ='" . mysqli_real_escape_string($ketnoi, $usercheck3b) . "' AND `magd` ='$magd'  AND `type` = 'ACB'")); // Check xem mã GD đã có chưa
        if ($dem < 1 && $create == 'on') {
          $cashmoi = $cashto + $amount;
          $type = "ACB";
          $statusne = "success";
          $note2 = "Chuyển tiền ACB hệ thống tự động cộng số dư cho user của bạn";
          // Lịch sử Nạp Tiền
        ...
        `date` = '$date'");
          ...
          echo 'DA CONG TIEN ROI<br>';
        }
      } else {
        echo "Không có đơn mới <br>";
      }
    } else {
