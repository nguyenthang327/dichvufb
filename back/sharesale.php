					...
					<div class="d-inline-block align-items-center">
						<nav>
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item" aria-current="page">Sub Rẻ</li>
							</ol>
						</nav>
					</div>
				</div>
		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			 <a style="float:right;" href="/history.html?type=13" class="btn btn-primary"><i class="fas fa-history"></i> Lịch sử đơn hàng</a>
			</div>
			<!-- /.box-header -->
			<div class="box-body">
			  <div class="row">
...
<div class="form-group row">
                          ...
                          <div class="col-sm-9">
<?php                              
$respawn = mysqli_query($ketnoi,"SELECT * FROM `chietkhau` WHERE `madichvu` = 'shareprofile' AND `webdinhdanh` = '$domain'");
...
<?php else: while ($row = mysqli_fetch_array($respawn, MYSQLI_ASSOC)):?>
  ...
  <?=$row['note']?> <div class="badge bg-primary"><?=$row[$order]?> Coin / 1 Share</div>
  </label>
</div>
     <?php endwhile; endif; ?>                                                 
                    </div>
...
<script type="text/javascript">
function submit(){if(!$('#idpost')['val']()){swal('ERROR','Bạn chưa nhập UID cần tăng share','error')}
