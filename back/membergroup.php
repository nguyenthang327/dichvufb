		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			 <a style="float:right;" href="/history.html?type=7" class="btn btn-primary"><i class="fas fa-history"></i> Lịch sử đơn hàng</a>
			</div>
			<!-- /.box-header -->
			<div class="box-body">
			  <div class="row">
...
<div class="form-group row">
                          <label for="idpost" class="col-sm-3 col-form-label">ID Group</label>
                          <div class="col-sm-9">
                            <input type="text" class="form-control" id="idpost"  name="idpost" placeholder="Nhập ID Group cần tăng">
                          </div>
                        </div>
                        <br>
<div class="form-group row">
                          ...
                          <div class="col-sm-9">
<?php                              
$respawn = mysqli_query($ketnoi,"SELECT * FROM `chietkhau` WHERE `madichvu` = 'membergroup' AND `webdinhdanh` = '$domain'");
...
<?php else: while ($row = mysqli_fetch_array($respawn, MYSQLI_ASSOC)):?>
  ...
  <?=$row['note']?> <div class="badge bg-primary"><?=$row[$order]?> Coin / 1 Member</div>
  </label>
</div>
     <?php endwhile; endif; ?>                                                 
                    </div>
...
<p>+ Hệ thống chạy bằng UID Group</p>
...
<p>+ Một Group chỉ được mua 1 đơn cùng lúc, đơn cũ chưa xong mà mua thêm sẽ lỗi .</p>
</div>
</div>
...
<script type="text/javascript">
function submit(){if(!$('#idpost')['val']()){swal('ERROR','Bạn chưa nhập UID cần tăng Member','error')}
