					...
					<div class="d-inline-block align-items-center">
						<nav>
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item" aria-current="page">Lịch Sử Chuyển Tiền</li>
							</ol>
						</nav>
					</div>
				</div>
		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			 <a style="float:right;" href="/transfer.html" class="btn btn-primary"><i class="fa fa-plus-circle fa-fw me-1"></i> CHUYỂN TIỀN</a>
			</div>
			<!-- /.box-header -->
			<div class="box-body">
			  <div class="row">
...
<li class="nav-item me-3"><a href="#allTab" class="nav-link active px-2" data-bs-toggle="tab">Transfer</a></li>
<li class="nav-item me-3"><a href="#Invoid" class="nav-link px-2" data-bs-toggle="tab">Invoid</a></li></ul>
...
<th class="border-top-0 pt-0 pb-2">Loại hình</th>
<th class="border-top-0 pt-0 pb-2">Số tiền</th>
<th class="border-top-0 pt-0 pb-2">Thực nhận</th>
<th class="border-top-0 pt-0 pb-2">Username nhận</th>
<th class="border-top-0 pt-0 pb-2">Thời gian chuyển</th>
...
$total_records = mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `historynaptien` WHERE `username` = '$username' AND `webdinhdanh` = '$domain' AND `matinhnang` = 'transfer' $seachans"));
$total_pages = ceil($total_records / $num_rec_per_page);                                            
$respawn = mysqli_query($ketnoi,"SELECT * FROM `historynaptien` WHERE `username` = '$username' AND `webdinhdanh` = '$domain' AND `matinhnang` = 'transfer' $seachans  order by id desc LIMIT $start_from, $num_rec_per_page ");
...
<td class="align-middle"><?=$row['usernhantien']?></td>
...
<div class="tab-pane fade show" id="Invoid">
<form method="GET" action="">
<div class="input-group">
										<input type="text" class="form-control" placeholder="seach" name="seach" id="seach" required="" aria-invalid="false"> 
										<button type="submit" class="btn btn-info btn-sm" type="button">Tìm kiếm</button> 
...
<th class="border-top-0 pt-0 pb-2">Số tiền</th>
<th class="border-top-0 pt-0 pb-2">Thực nhận</th>
<th class="border-top-0 pt-0 pb-2">Username chuyển</th>
<th class="border-top-0 pt-0 pb-2">Thời gian chuyển</th>
<th class="border-top-0 pt-0 pb-2">Trạng thái</th>
<th class="border-top-0 pt-0 pb-2">Action</th>
</tr>
...
$num_rec_per_page=100;
$start_from = ($page-1) * $num_rec_per_page;
$total_records = mysqli_num_rows(mysqli_query($ketnoi, "SELECT * FROM `historynaptien` WHERE `usernhantien` = '$username' AND `type` = 'invoid' AND `webdinhdanh` = '$domain' AND `matinhnang` = 'invoid' $seachans"));
$total_pages = ceil($total_records / $num_rec_per_page);                                            
$respawn = mysqli_query($ketnoi,"SELECT * FROM `historynaptien` WHERE `usernhantien` = '$username' AND `type` = 'invoid' AND `webdinhdanh` = '$domain' AND `matinhnang` = 'invoid' $seachans  order by id desc LIMIT $start_from, $num_rec_per_page ");
if (mysqli_num_rows($respawn) == 0):
?>    
