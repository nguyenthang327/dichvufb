					...
					<div class="d-inline-block align-items-center">
						<nav>
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item" aria-current="page">View Rẻ</li>
							</ol>
						</nav>
					</div>
				</div>
		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			 <a style="float:right;" href="/history.html?type=15" class="btn btn-primary"><i class="fas fa-history"></i> Lịch sử đơn hàng</a>
			</div>
			<!-- /.box-header -->
			<div class="box-body">
			  <div class="row">
...
<div class="form-group row">
                          <label for="idpost" class="col-sm-3 col-form-label">Link Story</label>
                          <div class="col-sm-9">
                            <input type="text" class="form-control" id="idpost"  name="idpost" placeholder="Nhập Link Story cần tăng">
                          </div>
                        </div>
                        <br>
<div class="form-group row">
                          ...
                          <div class="col-sm-9">
<?php                              
$respawn = mysqli_query($ketnoi,"SELECT * FROM `chietkhau` WHERE `madichvu` = 'viewstory' AND `webdinhdanh` = '$domain'");
...
<?php else: while ($row = mysqli_fetch_array($respawn, MYSQLI_ASSOC)):?>
  ...
  <?=$row['note']?> <div class="badge bg-primary"><?=$row[$order]?> Coin / 1 View</div>
  </label>
</div>
     <?php endwhile; endif; ?>                                                 
                    </div>
...
<p>+ Hệ thống chạy bằng Link Story</p>
<p>+ Nhập sai định dạng Link, không bật View cho tất cả sẽ không được hoàn tiền</p>
<p>+ Nếu gặp lỗi, quý khách vui lòng liên hệ hỗ  trợ trực tuyến</p>
<p>+ View nhảy sau khoảng 20-30p sau khi mua</p>
...
<p>+ Một vài bài share có thể sẽ lỗi không lên View, mọi người có thể qua mua View nhanh nhé</p>
...
<script type="text/javascript">
function submit(){if(!$('#idpost')['val']()){swal('ERROR','Bạn chưa nhập UID cần tăng View','error')}
