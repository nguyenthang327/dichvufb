					...
					<div class="d-inline-block align-items-center">
						<nav>
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item" aria-current="page">Like Rẻ</li>
							</ol>
						</nav>
					</div>
				</div>
		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			 <a style="float:right;" href="/history.html?type=16" class="btn btn-primary"><i class="fas fa-history"></i> Lịch sử đơn hàng</a>
			</div>
			<!-- /.box-header -->
			<div class="box-body">
			  <div class="row">
...
<div class="form-group row">
                          ...
                          <div class="col-sm-9">
<?php                              
$respawn = mysqli_query($ketnoi,"SELECT * FROM `chietkhau` WHERE `madichvu` = 'matlive' AND `webdinhdanh` = '$domain'");
...
<?php else: while ($row = mysqli_fetch_array($respawn, MYSQLI_ASSOC)):?>
  ...
  <?=$row['note']?> <div class="badge bg-primary"><?=$row[$order]?> Coin / 1 Mắt</div>
     ...
     <?php endwhile; endif; ?>                                                 
                    ...
                    <br>
                        <div class="form-group row">
                          <label for="amount" class="col-sm-3 col-form-label">Số mắt</label>
                          <div class="col-sm-9">
                            <input type="number" class="form-control" id="amount" name="amount" placeholder="50" onchange="checkpoint()">
                          </div>
                        </div>
                        <br>
                        <div class="form-group row">
                        <label for="exampleInputPassword2" class="col-sm-3 col-form-label">Số phút </label>
                        <div class="col-sm-9">
                        <input type="text" class="form-control" name="minutes" id="minutes" placeholder="50" placeholder="Nhập số phút cần tăng"  onchange="checkpoint()">
                        </div>
                            </div>
                        <br>    
                        <div class="form-group row">
...
<script type="text/javascript">
function submit(){if(!$('#idpost')['val']()){swal('ERROR','Bạn chưa nhập UID cần tăng Mắt','error')}
...
else {if(!$('#minutes')['val']()){swal('ERROR','Bạn chưa nhập số phút','error')}
