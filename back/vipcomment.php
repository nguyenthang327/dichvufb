...
<script type="text/javascript">
        function checkpoint() {
            var slbvtmn = $("#slbvtmn").val();
            var slclnct = $("#slclnct").val();
					...
					<div class="d-inline-block align-items-center">
						<nav>
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item" aria-current="page">Vip Comment</li>
							</ol>
						</nav>
					</div>
				</div>
		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			<div class="box-header with-border">
			 <a style="float:right;" href="/history.html?type=18" class="btn btn-primary"><i class="fas fa-history"></i> Lịch sử đơn hàng</a>
			</div>
			<!-- /.box-header -->
			<div class="box-body">
			  <div class="row">
...
<div class="form-group row">
                          ...
                          <div class="col-sm-9">
<?php                              
$respawn = mysqli_query($ketnoi,"SELECT * FROM `chietkhau` WHERE `madichvu` = 'vipcomment' AND `webdinhdanh` = '$domain'");
...
<?php else: while ($row = mysqli_fetch_array($respawn, MYSQLI_ASSOC)):?>
  ...
  <?=$row['note']?> <div class="badge bg-primary"><?=$row[$order]?> Coin / 1 Vip</div>
     ...
     <?php endwhile; endif; ?>                                                 
                    ...
                    <br>    
                     <div class="form-group row">
                         <label for="exampleInputEmail2" class="col-sm-3 col-form-label">Số lượng comment nhỏ nhất cần tăng:</label>
                         <div class="col-sm-9">
                        <input type="number" class="form-control" id="slcnnct" name="slcnnct" placeholder="2" >
                         </div> 
                         </div> <br> 
                         <div class="form-group row">
                         <label for="exampleInputEmail2" class="col-sm-3 col-form-label">Số lượng comment lớn nhất cần tăng:</label>
                         <div class="col-sm-9">
                        <input type="number" class="form-control" id="slclnct" name="slclnct" placeholder="5" onchange="checkpoint()">
                         </div> 
                         </div> <br> 
                         <div class="form-group row">
                         <label for="exampleInputEmail2" class="col-sm-3 col-form-label">Số lượng bài viết trong ngày:</label>
                         <div class="col-sm-9">
                        <input type="number" class="form-control" id="slbvtmn" name="slbvtmn" placeholder="2" onchange="checkpoint()">
                                                    <input type="hidden" class="form-control" id="bltk" name="bltk" value="all">
                         </div> 
                     ...
                     <select class="form-select mb-3" id="day_sale" name="day_sale" onchange="checkpoint()">
                         ...
                         <option value="90">90</option>
                                                 ...
                                                 <div class="form-group row">
                          <label for="exampleInputPassword2" class="col-sm-3 col-form-label">Nội dung comment</label>
                          <div class="col-sm-9">
                            <textarea rows="10" placeholder="Nhập nội dung seeding, mỗi dòng 1 nội dung được tính tiền là 1 lần seeding" class="form-control input-gray" id="ndr" name="ndr"></textarea>
                          </div>
                        </div><br>
                        <div class="form-group row">
...
else {if(!$('#slcnnct')['val']()){swal('ERROR','Bạn chưa chọn số lượng comment nhỏ nhất cần tăng','error')}
else {if(!$('#slclnct')['val']()){swal('ERROR','Bạn chưa chọn số lượng comment lớn nhất cần tăng','error')}
else {if(!$('#slbvtmn')['val']()){swal('ERROR','Bạn chưa chọn số lượng bài viết trong ngày','error')}
else {if(!$('#day_sale')['val']()){swal('ERROR','Bạn chưa chọn số ngày cần mua Vip','error')}
else {if(!$('#ndr')['val']()){swal('ERROR','Bạn chưa nhập nội dung comment','error')}
